...
- [ ] **Сущность «сессия клиента»** — UUID, реестр сессий, session ID в протоколе
- [ ] **Per-session isolation** — маршрутизация по session ID
- [ ] **Keepalive мониторинг** — детект мёртвых сессий
- [ ] **Reconnect с учётом сессии** — re-auth + восстановление TUN
- [ ] **Graceful shutdown сервера** — контекст + cleanup сессий
