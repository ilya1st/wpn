...
### ⚠️ Пинги не работают (сервер macOS ↔ клиент Linux)

**Дата обнаружения:** 10 Апреля 2026

**Симптомы:**
- Сервер на macOS запускается, интерфейс utun создаётся
- Клиент на Linux подключается, соединение устанавливается
- Пинги не ходят ни в одну сторону
- Фаервол отключён
- На Linux клиенте в tcpdump виден только: `ICMP6, router solicitation`

**Подозрения:**
- Неправильная маска/настройка `ifconfig` на macOS utun
- Возможно `/24` маска вместо point-to-point `/32`
- Возможно маршруты не добавляются корректно на macOS

**Что проверить:**
- `ifconfig utun0` — посмотреть реальную настройку интерфейса
- `netstat -rn` — проверить маршруты
- Попробовать `ifconfig utun0 <ip> <dest> netmask 255.255.255.255`
