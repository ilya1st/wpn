...
│   ├── tun/tun.go             # TUN интерфейс (создание, настройка)
...
**TUN интерфейс:** реализован через syscall `ioctl TUNSETIFF` (без внешних зависимостей)

## ✅ Реализовано

- [x] TUN интерфейс для IPv4 и IPv6
...
2. **Только Linux** — macOS и Windows не поддерживаются (syscall `TUNSETIFF` Linux-only)
3. **Статическая сборка** — всегда использовать `CGO_ENABLED=0 go build`
4. **TLS опционален** — можно использовать как `ws://` так и `wss://`
5. **Go 1.19** — зависимости совместимы с этой версией (не использовать новее без проверки)
6. **Маршруты** — клиентские маршруты имеют больший приоритет чем серверные (метрика +1000 к серверным)
