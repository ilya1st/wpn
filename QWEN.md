...
- [x] Восстановление соединения клиентом при разрыве (runSession/runWithReconnect, exponential backoff)
...
- [x] Добавление сжатия (флаг COMPRESSED)
- [x] Восстановление соединения клиентом при разрыве (автоматический reconnect)
- [ ] клиент при соединении по HTTPS должен маскироваться под реальные браузеры по TLS отпечаткам (fingerprinting) и поддерживать подмену TLS SNI
- [ ] Внешняя база пользователей (вместо plaintext в конфиге)
- [ ] Статистика по трафику на каждого пользователя
